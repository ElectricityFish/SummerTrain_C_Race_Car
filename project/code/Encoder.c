...
//读取左编码器在两次调用之间产生的脉冲数，读取后立即清零，适合定时速度计算。
...
//读取右编码器在两次调用之间产生的脉冲数，读取后立即清零，适合定时速度计算。
int16 encoder_right_get_pulse(void)
{
	int16 pulse = -encoder_get_count(ENCODER_RIGHT_TIMER);	//这里有个负号，使轮胎向前转时脉冲为正
