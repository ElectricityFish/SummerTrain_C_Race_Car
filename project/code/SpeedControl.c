...
#define SPEED_CONTROL_DEFAULT_OUT_MAX       (3000)
...
static void speed_control_update_pid(volatile Speed_PID_t *pid)
{
    ...
    // 菜单参数采用 0~10 的易调范围，内部统一放大 10 倍以适配 0~10000 的 PWM 标度。
    pid->POut = SPEED_CONTROL_MENU_GAIN_SCALE * pid->Kp * pid->Error;
    ...
    else
    {
        proposed_error_int = speed_control_limit_float(
            ...
            SPEED_CONTROL_INTEGRAL_ABS_MAX);
        output = pid->POut + SPEED_CONTROL_MENU_GAIN_SCALE * pid->Ki * proposed_error_int;

        // 输出已到上限且误差仍推动饱和时暂停积分，防止松开后反向冲击。
    ...
    pid->IOut = SPEED_CONTROL_MENU_GAIN_SCALE * pid->Ki * pid->ErrorInt;
...
void speed_control_init(void)
{
    ...
    speed_left_pid.Kp = 2.50f;
    speed_left_pid.Ki = 0.02f;
    ...
    speed_right_pid.Kp = 3.50f;
    speed_right_pid.Ki = 0.02f;
